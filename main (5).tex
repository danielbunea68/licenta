...
(Medical Open Network for AI) și urmează o ordine strict definită de transformări deterministe
...
kernelurile de convoluție $3{\times}3{\times}3$ tratează toate cele trei dimensiuni în mod egal,
...
astfel încât rețeaua să opereze într-un spațiu metric consistent. Alegerea valorii de spacing
implică un compromis important între rezoluție și dimensiunea memoriei GPU necesare.
...
vase), eliminând contribuția structurilor irelevante (os, aer).
...
Transformul \texttt{Orientationd} din MONAI aplică automat permutările și flip-urile necesare
...
\texttt{CropForegroundd} calculează cel mai mic paralelipiped dreptunghic (bounding box 3D)
...
\texttt{RandCropByPosNegLabeld} rezolvă această problemă prin controlul explicit
...
Sliding window cu overlap rezolvă această problemă prin extragerea
...
(nnUNet). Ele simulează variațiile naturale ale poziționării pacientului
...
inevitabile în practica clinică.
...
geometrică. Se aplică independent pe fiecare din cele trei axe
...
într-o singură operație, configurabilă independent pe fiecare
componentă:
...
interpolare, \texttt{RandRotate90d} produce exclusiv rotații de
...
această variabilitate în timpul antrenamentului, făcând modelul mai robust la aceste diferențe.
...
Adaugă zgomot gaussian aleatoriu peste fiecare voxel al imaginii CT.
...
din intervalul $[0.5, 1.5]$ voxeli. Blur-ul simulează efectul de „înmuiere" a marginilor
...
Înmulțește toate intensitățile din volum cu un factor aleatoriu în intervalul $[0.7, 1.3]$.
...
$I \in [0, 1]$ la puterea $\gamma$: $I' = I^{\gamma}$.
...
folosind interpolarea trilineară, apoi îl readuce la dimensiunea originală prin upsampling.
...
10 puncte de control aleatoare. Este cea mai complexă augmentare de intensitate din pipeline:
...
la toate intensitățile din volum. Simulează \textit{bias field}-ul — o variație lentă
...
care măsoară suprapunerea dintre predicția modelului și \textit{ground truth}.
\end{infobox}
...
pe suprapunerea dintre clasele de interes, nu pe numărul de voxeli din fiecare clasă.
...
reprezentări utile — nu doar stratul final.
\end{infobox}
...
a ratei de învățare, corectând o problemă fundamentală din Adam original.
\end{infobox}
...
gradient, producând o regularizare uniformă și mai eficientă.
...
problemă prin \textbf{loss scaling}.
