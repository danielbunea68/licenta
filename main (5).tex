...
\begin{tabular}{p{4.0cm} p{2.5cm} p{2.8cm} p{2.8cm}}
    ...
    \midrule
    MA-cGAN \newline (2024)~\cite{macgan2024}
        & LiTS & 0.969 & 0.785 \\
    \addlinespace
    ELANRes-MSCA-UNet \newline (2025)~\cite{elanres2025}
        & LiTS & 0.972 & 0.729 \\
    \addlinespace
    H-DenseUNet \newline (Li et al., 2018)~\cite{li2018hdenseunet}
        & LiTS & 0.961 & 0.722 \\
    \addlinespace
    nnUNet \newline (Isensee et al.)~\cite{isensee2019automated}
        & LiTS & 0.967 & 0.763 \\
    \addlinespace
    G-UNETR++ $^{\dagger}$~\cite{gunetrpp}
        & LiTS & — & — \\
    \addlinespace
    SwinUNETR \newline (Tang et al., 2022)~\cite{tang2022swinunetr}
        & MSD Task03 & 0.954 & 0.757 \\
    \addlinespace
    \textbf{Monolitic (această lucrare)}
        & LiTS & \textbf{0.958} & \textbf{0.753} \\
    \addlinespace
\textbf{Cascade (această lucrare)}
        & LiTS & \textbf{0.9521 } & \textbf{0.6869 } \\
    \bottomrule
\end{tabular}
\end{center}
...
(Dice 0.958, comparabil cu nnUNet 0.967~\cite{isensee2019automated})
și pentru segmentarea tumorilor (Dice 0.753, comparabil cu SwinUNETR
0.757 raportat de Tang et al.~\cite{tang2022swinunetr} pe MSD
Task03 Liver). Totuși, o comparație corectă cu metodele de referință
impune analiza critică a condițiilor în care aceste scoruri au fost
obținute --- în special diferențele de dataset (LiTS vs.\ MSD
Task03 Liver) și de protocol de evaluare.
...
și masca reală (\textit{Ground Truth}), exprimată ca un scor între 0 și 1~\cite{bilic2023lits}.
\end{infobox}
...
și mai variabile (0.50–0.80), reflectând dificultatea problemei~\cite{bilic2023lits}.
...
exprimată ca eroare procentuală. Este complementul lui IoU (Jaccard Index)~\cite{bilic2023lits}.
\end{infobox}
...
voxelii — doar de \textit{câți} sunt~\cite{bilic2023lits}.
\end{infobox}
...
nu pe volume~\cite{bilic2023lits}.
\end{infobox}
...
decât cele mari, sunt raportate separate metrici de detecție per categorie~\cite{bilic2023lits}:
...
este \textbf{FP} (detecție falsă)~\cite{bilic2023lits}.
...
2021~\cite{isensee2019automated, isensee2018selfadapting}. Filozofia nnUNet nu este aceea de a
...
medicale 3D. Arhitectura a fost propusă de Tang et al.\ (2022)~\cite{tang2022swinunetr} și reprezintă
...
și batch size mic~\cite{isensee2019automated}.
...
Strategia de augmentare urmează protocolul complet nnUNet~\cite{isensee2019automated},
...
\bibitem{bilic2023lits}
Bilic, P., Christ, P., Li, H. B., Vorontsov, E., Ben-Cohen, A., Kaissis, G.,
Szeskin, A., Jacobs, C., Mamani, G. E. H., Chartrand, G., et al. (2023).
...
(arXiv preprint: \url{https://arxiv.org/abs/1901.04056})
